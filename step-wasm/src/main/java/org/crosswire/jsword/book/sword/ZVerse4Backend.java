...
import org.crosswire.jsword.book.sword.state.WasmRandomAccessFile;
 ...
 * <li>Verse length - 32 bit/4-bytes - the length of the verse in the uncompressed block from the dat file.</li>
 * </ul>
 * Algorithm:
 * <ul>
 * <li>Given the ordinal value of the verse, seek to the ordinal * 12 and read 12 bytes.
 * <li>Decode the 12 bytes as Block Number, Verse start and length</li>
 * </ul>
 * </li>
 ...
 * @author DM Smith
 */
public class ZVerse4Backend extends AbstractBackend<ZVerseBackendState> {
    /**
     * Simple ctor
     */
    public ZVerse4Backend(SwordBookMetaData sbmd, BlockType blockType) {
        super(sbmd);
    ...
    public int getRawTextLength(Key key) {
        ...
        try {
            ...
            WasmRandomAccessFile idxRaf = rafBook.getIdxRaf(testament);
    ...
    public Key getGlobalKeyList() throws BookException {
        ...
        try {
            ...
            for (Testament currentTestament : testaments) {
                WasmRandomAccessFile idxRaf = rafBook.getIdxRaf(currentTestament);
    ...
    public String readRawContent(ZVerseBackendState rafBook, Key key) throws IOException {
        ...
        int origOrdinal = verse.getOrdinal();
        ...
        if (verseFromAugmentedFile != null)
            return verseFromAugmentedFile; // Found it in cache

        final WasmRandomAccessFile idxRaf;
        final WasmRandomAccessFile compRaf;
        final WasmRandomAccessFile textRaf;
        ...
        //dumpCompRaf(idxRaf);
        // 10 because the index is 10 bytes long for each verse
        byte[] temp = SwordUtil.readRAF(idxRaf, 1L * ordinalInTestament * IDX_ENTRY_SIZE, IDX_ENTRY_SIZE);
        ...
        final int verseSize = SwordUtil.decodeLittleEndian32(temp, 8);
     ...
     * @param raf
     */
    public void dumpIdxRaf(Versification v11n, int ordinalStart, WasmRandomAccessFile raf) {
        long end = -1;
        try {
            end = raf.length();
        } catch (Throwable e) {
     ...
     * @param raf
     */
    public void dumpCompRaf(WasmRandomAccessFile raf) {
        long end = -1;
        try {
            end = raf.length();
        } catch (Throwable e) {
     ...
     * The log stream
     */
    private static final Logger log = LoggerFactory.getLogger(ZVerse4Backend.class);
