...
public class ZVerseBackendState extends AbstractOpenFileState {
     ...
     */
     ZVerseBackendState(SwordBookMetaData bookMetaData, BlockType blockType) throws BookException {
        ...
        if (otIdxFile.canRead()) {
            try {
                otCompRaf = new WasmRandomAccessFile(otIdxFile, FileUtil.MODE_READ);
                otTextRaf = new WasmRandomAccessFile(otTextFile, FileUtil.MODE_READ);
                otIdxRaf = new WasmRandomAccessFile(otCompFile, FileUtil.MODE_READ);
        ...
        if (ntIdxFile.canRead()) {
            try {
                ntCompRaf = new WasmRandomAccessFile(ntIdxFile, FileUtil.MODE_READ);
                ntTextRaf = new WasmRandomAccessFile(ntTextFile, FileUtil.MODE_READ);
                ntIdxRaf = new WasmRandomAccessFile(ntCompFile, FileUtil.MODE_READ);
            } catch (FileNotFoundException ex) {
                //failed to open the files, so close them now
     ...
     */
    public WasmRandomAccessFile getCompRaf(Testament testament) {
     ...
     * @return the index for the testament
     */
    public WasmRandomAccessFile getTextRaf(Testament testament) {
     ...
     * @return the index for the testament
     */
    public WasmRandomAccessFile getIdxRaf(Testament testament) {
     ...
     */
    private WasmRandomAccessFile otCompRaf;
    private WasmRandomAccessFile ntCompRaf;

    /**
     * The data random access files
     */
    private WasmRandomAccessFile otTextRaf;
    private WasmRandomAccessFile ntTextRaf;

    /**
     * The index random access files
     */
    private WasmRandomAccessFile otIdxRaf;
    private WasmRandomAccessFile ntIdxRaf;
