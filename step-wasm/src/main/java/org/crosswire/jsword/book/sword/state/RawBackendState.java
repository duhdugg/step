...
public class RawBackendState extends AbstractOpenFileState {
    ...
    RawBackendState(SwordBookMetaData bookMetaData) throws BookException {
        ...
        if (otIdxFile.canRead()) {
            try {
                otIdxRaf = new WasmRandomAccessFile(otIdxFile, fileMode);
                otTextRaf = new WasmRandomAccessFile(otTextFile, fileMode);
        ...
        if (ntIdxFile.canRead()) {
            try {
                ntIdxRaf = new WasmRandomAccessFile(ntIdxFile, fileMode);
                ntTextRaf = new WasmRandomAccessFile(ntTextFile, fileMode);
            } catch (FileNotFoundException ex) {
                //failed to open the files, so close them now
     ...
     */
    public WasmRandomAccessFile getOtTextRaf() {
        return otTextRaf;
    }
     ...
     */
    public WasmRandomAccessFile getNtTextRaf() {
        return ntTextRaf;
    }
    ...
    protected WasmRandomAccessFile otIdxRaf;
    protected WasmRandomAccessFile ntIdxRaf;
    protected WasmRandomAccessFile otTextRaf;
    protected WasmRandomAccessFile ntTextRaf;
