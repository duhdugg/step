...
public final class SwordUtil {
     ...
     *             on error
     */
    protected static byte[] readRAF(WasmRandomAccessFile raf, long offset, int theSize) throws IOException {
        raf.seek(offset);
        return readNextRAF(raf, theSize);
    }

    /**
     * Read a WasmRandomAccessFile from the current location in the file.
     * 
     * @param raf
     *            The file to read
     * @param theSize
     ...
     *             on error
     */
    protected static byte[] readNextRAF(WasmRandomAccessFile raf, int theSize) throws IOException {
...
//            log.info("Attempt to read beyond end. offset={} size={} but raf.length={}", Long.toString(offset), Integer.toString(size), Long.toString(rafSize));
    ...
    /**
     * Writes "data" to a WasmRandomAccessFile at the "offset" position
     * 
     * @param raf
     *            WasmRandomAccessFile
     * @param offset
     *            offset to write at
     * @param data
     *            data to write
     ...
     *             on error
     */
    protected static void writeRAF(WasmRandomAccessFile raf, long offset, byte[] data) throws IOException {
        raf.seek(offset);
        writeNextRAF(raf, data);
    }

    protected static void writeNextRAF(WasmRandomAccessFile raf, byte[] data) throws IOException {
        if (data == null) {
            return;
        }
        raf.write(data);
    ...
    /**
     * Read a WasmRandomAccessFile until a particular byte is seen
     * 
     * @param raf
     *            The file to read
     * @param offset
     ...
     *             on error
     */
    protected static byte[] readUntilRAF(WasmRandomAccessFile raf, int offset, byte stopByte) throws IOException {
        raf.seek(offset);
     ...
     *             on error
     */
    protected static byte[] readUntilRAF(WasmRandomAccessFile raf, byte stopByte) throws IOException {
